...
class _GestureZoomBoxState extends State<GestureZoomBox> with TickerProviderStateMixin {
  ...
  _scaling(ScaleUpdateDetails details) {
    ...
    double scaleOffsetX = context.size.width * (scale - 1.0) / 2;
    double scaleOffsetY = context.size.height * (scale - 1.0) / 2;
    // 将缩放前的触摸点映射到缩放后的内容上
  ...
  _dragging(ScaleUpdateDetails details) {
    ...
    // 处理 Y 轴边界
    double scaleOffsetY = (context.size.height * scale - MediaQuery.of(context).size.height) / 2;
    if (scaleOffsetY <= 0) {
      offsetYIncrement = 0;
